...
function EditPostComponent() {
    ...
    useEffect(() => {
        fetch(`http://52.196.111.244:3000/api/v1/users/${user_id}/microposts/${id}`, {
          method: 'GET',
          headers: {
  ...
  // useEffectの依存配列にuser_idを追加
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            const response = await fetch(`http://52.196.111.244:3000/api/v1/users/${user_id}/microposts/${id}`, {
                method: 'PATCH',
                headers: {
                    'Content-Type': 'application/json',
                    //jwt認証方法のため追加致しました。
